...
const PlanetsViewQuery = gql`
  query PlanetsViewQuery($skip: Int) {
    allPlanets(orderBy: name_ASC, first: 22, skip: $skip) {
...
class PlanetsView extends connect(store)(PageViewElement) {
  ...
  render() {
    const hasPlanets =
      !!this._response &&
      !!this._response.data &&
      !!this._response.data.allPlanets;

    const planets = hasPlanets ? this._response.data.allPlanets : [];
    ...
    const total =
      (hasPlanets &&
        this._response.data._allPlanetsMeta &&
        this._response.data._allPlanetsMeta.count) ||
      0;

    return html`
      <graphql-query
        id="PlanetsViewQuery"
        .query=${PlanetsViewQuery}
          ...
          : html`
              <section>
                <div class="split-list">
                  <div>
                    ${planets
                      .slice(0, Math.ceil(planets.length / 2))
                      .map(renderPlanet)}
                  </div>
                  <div>
                    ${planets
                      .slice(Math.ceil(planets.length / 2))
                      .map(renderPlanet)}
                  </div>
                </div>
              </section>
                ...
                <div class="page-buttons">
                  ${parsedPage <= 1
                    ...
                    : html`
                        <a
                          class="page-button"
                          href="/planets?page=${parsedPage - 1}"
                          >Previous</a
                        >
                      `}
                  ${parsedPage * 22 >= total
...
window.customElements.define("planets-view", PlanetsView);
